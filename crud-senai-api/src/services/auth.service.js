...
// Define o número máximo de tentativas de login falhadas (padrão: 3) a partir de 
//variáveis de ambiente 
const MAX = Number(process.env.MAX_LOGIN_ATTEMPTS || 3); 
// Define o tempo (em minutos) que um usuário fica bloqueado após exceder o 
//máximo de tentativas (padrão: 5) 
const LOCK_MINUTES = Number(process.env.LOCK_MINUTES || 5); 
// Função auxiliar que retorna uma data futura adicionando X minutos ao tempo npatual 
// Usada para definir quando um usuário bloqueado poderá tentar novamente 
function nowPlusMinutes(min) { 
// Cria uma nova data com o tempo atual + (minutos * 60 segundos * 1000 
//milissegundos) 
return new Date(Date.now() + min * 60 * 1000); 
} 
// Função assíncrona principal de login com mecanismo de bloqueio por 
//tentativas falhadas 
...
// SELECT retorna ID, nome, email, hash da senha, perfil, status, tentativas 
//falhadas e data de desbloqueio 
...
if (!passOk) { 
// Incrementa o contador de tentativas falhadas, com máximo de 255 para não 
//exceder limite do banco 
...
// Opções: o token expira em 1 hora (ou valor configurado em variável de 
//ambiente) 
